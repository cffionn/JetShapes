...
inline std::vector<float> commaSepStringToVectF(std::string inStr)
{
  ...
  std::string strToSep = inStr;
  while(inStr.find(",") != std::string::npos){
    std::string numStr = inStr.substr(0, inStr.find(","));
    if(!isStrFloatOrDouble(numStr)){
      std::cout << __PRETTY_FUNCTION__ << ": Given '" << strToSep << "' is not all float. return {}" << std::endl;
      return {};
    }
    retVect.push_back(std::stof(numStr));
    inStr.replace(0, inStr.find(",")+1, "");
  }
